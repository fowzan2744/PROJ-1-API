...
const testClientEndpoints = async () => {
  ...
  try {
    ...
    const statsResponse = await axios.get(`${BASE_URL}/dashboard/stats?merchant_id=M001`);
    ...
    const stepResponse = await axios.get(`${BASE_URL}/dashboard/transactions-by-step?merchant_id=M001`);
    ...
    const customerResponse = await axios.get(`${BASE_URL}/dashboard/amount-by-customer?merchant_id=M001`);
    console.log('✅ Client amount by customer working:', customerResponse.data.length, 'customers');

    // Test fraud stats
    console.log('\n4. Testing /dashboard/fraud-stats...');
    const fraudResponse = await axios.get(`${BASE_URL}/dashboard/fraud-stats?merchant_id=M001`);
    ...
    const transactionsResponse = await axios.get(`${BASE_URL}/dashboard/transactions?page=1&limit=10&merchant_id=M001`);
    ...
    const categoryResponse = await axios.get(`${BASE_URL}/dashboard/category-breakdown?merchant_id=M001`);
